...
- 번호 생성: 로또 6/45 조합 생성, 고정/제외 번호, 합계·연속·홀짝 제한, QR 생성
- 번호 추천: 다중 전략, 자동 전략 선택, worker 기반 후보 분석, 추천 근거 표시
- 연금복권720+: 공식 데이터 기반 통계, 조/자리별 빈도, 추천 번호, 저장 번호 CSV export
- 당첨 확인: 저장된 로또/연금복권 번호를 최신 결과와 비교
- 데이터 관리: 즐겨찾기, 히스토리, 로또 연습, 캠페인, 프로필, 연금복권 저장 번호 백업/복원
- PWA: 홈 화면 설치, app shell/data precache, 업데이트 확인/적용, 캐시 상태 표시
...
- 기존 `lotto_pro_*` localStorage key는 사용자 데이터 호환성을 위해 유지합니다.
- localStorage 저장 실패가 발생하면 dirty 상태를 유지하고, 마지막 저장 실패 정보를 설정/상태 화면에 경고로 표시합니다.
- Backup schema는 v4입니다. 기본 export prefix는 `lotto_pension_pro_backup_v4`입니다.
...
- generated/AI/Pension720 임시 결과는 `lotto_pro_temp_results_state` sessionStorage에만 저장합니다. 이 데이터는 backup v4나 localStorage 영구 저장 대상에 포함하지 않습니다.
