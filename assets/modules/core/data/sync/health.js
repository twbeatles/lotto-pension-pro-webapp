...
export const dataSyncHealthMethods = {
    assessWinningStatsStructure(items = []) {
        ...
        (Array.isArray(items) ? items : []).forEach((item) => {
            const drawNo = Math.max(0, Math.floor(Number(item?.draw_no || 0)));
            if (!drawNo) return;
        ...
        const allowedMissing = new Set((CONFIG.LIMITS.MISSING_DRAWS || [])
    ...
    summarizeMissingDraws(drawNos = []) {
        const list = [...new Set((Array.isArray(drawNos) ? drawNos : []).map((value) => Math.floor(Number(value) || 0)).filter(Boolean))];
    ...
    getWinningStatsDataHealth({ staticItems = [], localUpdates = [], mergedItems = [], staticError = null } = {}) {
        ...
        if (staticAvailable && staticStructure.isStructurallyComplete) {
        ...
        if (latestDrawNo > 0) {
            const source = staticAvailable
                ? (localAvailable ? 'static_local' : 'static')
                : (localAvailable ? 'local_only' : 'none');
            const missingSummary = this.summarizeMissingDraws(staticStructure.unexpectedMissing);
            return this.mergeDataHealth({
                availability: 'partial',
                source,
                latestDrawNo,
                message: staticAvailable
                    ? `정적 JSON 전체 데이터가 완전하지 않아 일부 데이터만 사용 중입니다.${missingSummary ? ` (누락 회차: ${missingSummary})` : ''}`
                    : localAvailable
