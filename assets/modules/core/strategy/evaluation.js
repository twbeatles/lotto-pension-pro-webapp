...
export const strategyEvaluationMethods = {
    scoreSetCandidate(numbers, request, options = {}) {
        ...
        const normalized = options.normalizedRequest || options.execution?.normalizedRequest || this.normalizeRequest(request);
        ...
        const ctx = options.context || computed.context || this.buildContext(sourceData, normalized.params.lookbackWindow);
        ...
        const gapBalanceScore = candidate.reduce((acc, n) => {
        ...
        const profileScore = (sumScore * 0.32) + (acScore * 0.28) + (zoneCoverage * 0.20) + (endDigitScore * 0.10) + (carryScore * 0.10);
    ...
    explainSet(numbers, request, options = {}) {
        ...
        const { weights, context: ctx, adaptive } = this.computeWeightsFromNormalized(normalized, sourceData, {
            context: options.context
        });
