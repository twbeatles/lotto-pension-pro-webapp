...
2026-05-22 기준 `git check-ignore -v`로 백업 JSON, Pension720+/시뮬레이션 CSV, Playwright 산출물, report/perf 폴더, trace/HAR/video, dependency/temp/build 폴더의 대표 경로 매칭을 확인했습니다. `.github/workflows/data-freshness.yml`은 운영 workflow이므로 배포 브랜치에 포함합니다.
...
앱 실행 중에는 Lotto 6/45와 Pension720+ 데이터를 각각 런타임 동기화합니다. 실패 시 포함된 정적 데이터, 기존 메모리 데이터, 로컬 업데이트 또는 official cache 기반 상태를 유지합니다.
...
- 현재 `sw.js` cache version: `v28`
- 현재 strategy worker asset query version: `v22`
- cache names:
    - `lotto-pension-pro-app-shell-v28`
    - `lotto-pension-pro-data-v28`
...
4. DevTools > Application > Service Workers에서 `v28` 활성화 확인
...
기존 URL 또는 기존 설치 앱 사용자는 새 repository slug와 service worker scope/start URL을 바로 받지 못할 수 있습니다.

1. 기존 앱에서 `데이터 관리` > `전체 데이터 내보내기`로 백업 파일 저장
...
- `.github/workflows/data-freshness.yml`은 자동 커밋 없이 scheduled/manual freshness check와 `npm run build:release`를 실행합니다.
